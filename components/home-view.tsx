...
function AlertCard({ title, description, aligned, nonAligned }: {
  title: string;
  description: string;
  aligned: number;
  nonAligned: number;
...
export const HomeViewComponent: React.FC<HomeViewComponentProps> = ({ onNavigateToComplianceMapper }) => {
  ...
  return (
    <div className="space-y-6 relative p-4 sm:p-6 md:p-8">
      <div className="flex justify-between items-center">
        <h1 className="text-3xl font-bold">Security Resource Alignment</h1>
      </div>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <AlertCard
              ...
              description="The latest revision of the Information Security Policy has resulted in misalignment with the CRI Profiles control frameworks and 3 regulations."
              aligned={94}
              nonAligned={13}
            />
            <AlertCard
              title="CRI Profiles v2.0"
              description="A new version of CRI Profiles security control framework was recently published. The Information Security Policy is misaligned with 4 controls."
              aligned={96}
              nonAligned={4}
            />
          </div>
